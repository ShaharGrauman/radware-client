...
class WizardFooterButtons extends Component {
    render() {
        if(this.props.isUpdateSignature){
            return (
                <div style={{ width: '100%', boxShadow: 'inset 0px 1px 3px 0px rgba(20,20,20,1)', backgroundColor: '#007BFF', paddingTop: '20px', paddingBottom: '20px', margin: '0px', position: 'fixed', left: '0px', bottom: '0px' }}>
                    <FooterButton action={this.props.isCancelResarcherDashboard} args={{}} text="CANCEL" bootstrapClass="btn btn-dark ml-2"></FooterButton>
                    <FooterButton action={this.props.updateSignatureButton} args={{}} text="UPDATE SIGNATURE" bootstrapClass="btn btn-dark ml-2"></FooterButton>
                </div>
            );
        }
        return (
